...
const Navbar = () => {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 glass-dark border-b border-white/10">
      ...
      {/* Mobile menu */}
      {open && (
        <div className="md:hidden glass-dark border-b border-white/10 px-6 pb-6 pt-2 space-y-4">
          {links.map((l) => (
            <a
              key={l.href}
              href={l.href}
              onClick={() => setOpen(false)}
              className="block text-sm font-medium text-white/70 hover:text-secondary transition-colors py-2"
            >
              {l.label}
            </a>
          ))}
